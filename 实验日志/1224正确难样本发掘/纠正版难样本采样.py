...
class MOSE(object):
    ...
    def _sample_hard_from_buffer(self, batch_size, exclude_task_id=None, target_device=None):
        ...
        # 修正代码：优先读取 bu，如果没有再尝试 u 或 uncertainty
        if hasattr(self.buffer, 'bu'):
            buf_u = self.buffer.bu[:current_len]
        elif hasattr(self.buffer, 'u'):
            # 注意：self.buffer.u 是个 property，通常已经切片过了，需要确认类型
            # 但根据 buffer(1).py，self.u 返回的就是 self.bu[:current_index]
            # 所以如果用 self.u，就不需要再 [:current_len] 了，但这取决于 current_len 是否等于 self.current_index
            # 最稳妥是直接用 bu
            buf_u = self.buffer.u
            if len(buf_u) > current_len:  # 防御性编程
                buf_u = buf_u[:current_len]
        else:
            # 兼容其他类型的 buffer 或者 fallback
            buf_u = self.buffer.uncertainty[:current_len] if hasattr(self.buffer, 'uncertainty') else torch.zeros(
                current_len, device=ref_device)
            print("hasattr(self.buffer, 'uncertainty')")
            print("1223修改错误")
